...
class TFLiteObjectDetector implements ObjectDetector {
  ...
  Future<List<ObjectDetection>> _detectWithYOLO(img.Image image, double confThreshold) async {
    ...
    // Build [84][8400] output
    final output = List.generate(84, (_) => List<double>.filled(8400, 0.0));

    _interpreter!.run(input, [output]);

    return _postProcess(output, origW, origH, confThreshold);
  }

  // NHWC input [1, 640, 640, 3]
